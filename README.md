...
| SAM パラメータ | 既定値 | 動作 |
| --- | --- | --- |
| `FaqHttpApiFaqRoutesEnabled` | 文字列 `'true'` | `'true'` / `'false'`。shell の `FAQ_HTTP_API_FAQ_ROUTES_ENABLED` に反映。`false` は旧 HTTP API の FAQ POST に CORS ヘッダ付きの `410 {"error":"legacy_entrance_disabled"}` を即返します。4ルート・呼出し権限・POST RouteSettings は保持し、OPTIONS は従来どおり204です。`false` でも未認証の公開 POST は Lambda に到達し、起動に伴う課金・ログが発生します |
| `FaqHttpApiFaqThrottlingRateLimit` | 2 | 旧 FAQ の2つの POST route の requests/秒。整数1〜100。BurstLimit とともに停止後も Lambda 起動を抑える唯一のガードなので、低い値（推奨1）を維持し、既定2に戻しません |
| `FaqHttpApiFaqThrottlingBurstLimit` | 5 | 旧 FAQ の2つの POST route の burst。整数1〜100。RateLimit とともに停止後も Lambda 起動を抑える唯一のガードなので、低い値（推奨2）を維持し、既定5に戻しません |

FAQ の4つの HttpApi event は `FaqChatFunction.Events` に無条件で定義し、`sam local start-api` でも従来どおり検出できます。停止判定は shell の入口種別 `entrypoint=http-api` に限定し、入力検証・Settings 読み込み・処理枠取得・router より前に返します。router・検索・生成・処理枠を消費せず、`faq_shell_timing` には `entrypoint=http-api` と `handler_outcome=skipped` を記録します。REST STREAM 入口には影響しません。`FaqChatFunction` と IAM role は保持し、再有効化時に remote のクロスアカウント trust が参照する principal を作り直しません。Slack の設定有無にかかわらず停止でき、Slack の event と統合は従来どおりです。

停止後も Lambda に到達した要求ごとに `faq_shell_timing` が1行出力されます（`entrypoint=http-api`・`handler_outcome=skipped`）。これは0件確認と残存トラフィック検知の材料です。ボット等でログが増える場合は、ログ保持期間で対処してください。

throttle の2パラメータは `Number` 型です。`AllowedPattern` は `Number` に使えないため、`MinValue: 1` / `MaxValue: 100` と `AllowedValues` の整数1〜100で範囲と整数性を検証します。停止は `FaqHttpApiFaqRoutesEnabled=false` で行います。既定値と環境変数未設定時は旧 HTTP API の FAQ 処理を有効に保ちます。

1. 切替前の throttle / routes / caller trust を記録します。移行観測用の推奨値は `FaqHttpApiFaqThrottlingRateLimit=1` / `FaqHttpApiFaqThrottlingBurstLimit=2` です。数日間 `faq_shell_timing` の `entrypoint=http-api` 件数を観測します。REST STREAM 側の throttle は変わりません。
   rate 1 / burst 1 も指定できます。整数のため 1 rps 未満の段階は作れません。
2. 旧入口の利用件数が0になったら、`FaqHttpApiFaqRoutesEnabled=false` で再デプロイします。HTTP API の FAQ ルートと Output `FaqApiUrl` は残り、FAQ POST は shell が410を即返します。入力検証・Settings 読み込み・router・検索・生成・処理枠取得は行いません。OPTIONS は204を返し、Slack と REST STREAM は従来どおり動作します。
   `false` でもルートと Lambda 起動は残り、未認証の公開 POST が Lambda に到達して課金・ログが発生します。起動を抑える唯一のガードは `FaqHttpApiFaqThrottlingRateLimit` / `FaqHttpApiFaqThrottlingBurstLimit` なので、停止後も低い値（推奨1 / 2）のまま維持し、既定2 / 5に戻さないでください。
3. 停止後は UI の CSP `connect-src` から旧 origin を削除できます。
4. 切り戻しは `FaqHttpApiFaqRoutesEnabled=true` へ戻して再デプロイします。**時間延長後の切り戻しは前述の UI 切替手順4を守り**、MIF の時間短縮・処理中要求の完了・shell の短縮後に旧入口を再有効化し、記録した throttle / caller trust を確認・必要なら復元します。旧 origin を CSP から削除済みの場合は、許可する CSP の配信・キャッシュ更新を先に完了してから UI の URL と待機時間を戻します。
